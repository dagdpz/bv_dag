...
defpar = { ...
    ...
    'use_voi_name_only','logical', 'nonempty', false; ...		% don't add "Cluster..." to resulting voi name
...
if isxff(vmp_path),
    ...
    [vmp_pathstr,vmp_name,dummy] = fileparts(vmp_path);
end
...
for v = 1:voi.NrOfVOIs,
    ...
    [cs_ct, ctab_ct, vmp_ct, voi_ct] = vmp_voi_masked.ClusterTable(1, [],...
    ...
    if ~isempty(cs_ct),
        ...
        if N > 1 && params.combine_clusters,
            ...
            if params.use_voi_name_only
                voi_ct_sel.VOI(end).Name = [subj_str voi(v).Name];
            else
            ...
            for c = 1:N,
                if params.use_voi_name_only
                    voi_ct_sel.VOI(c).Name = [subj_str voi(v).Name];
                else
