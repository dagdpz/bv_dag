function prt_fname = mat2prt_fixstim_rest(mat_file, run_name)
...
for t = idx_abort % loop through all aborted trials
    if strcmp(trial(t).abort_code, 'ABORT_EYE_FIX_HOLD_STATE') == 0
...
if ~isempty(states_onset_abort)
    prtpreds(3).onset = states_onset_abort(:,1); % from beginning of trial
    prtpreds(3).offset = states_onset_abort(:,2) + ITI_fail; % to end of trial incl. ITI
else
    prtpreds(3).onset = [];
    prtpreds(3).offset = [];
end
