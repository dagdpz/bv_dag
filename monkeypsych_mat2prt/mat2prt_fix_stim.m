function prt_fname = mat2prt_fix_stim(mat_file, run_name)
...
prtpreds(3).name = 'fixation microstim';
prtpreds(3).r = 0; %255;
prtpreds(3).g = 160; %255;
prtpreds(3).b = 0;

% prtpreds(4).name = '4 post fixation microstim';
prtpreds(4).name = 'post fixation microstim';
prtpreds(4).r = 255;
prtpreds(4).g = 181;
prtpreds(4).b = 97;

% prtpreds(13).name = '13 reward';
prtpreds(5).name = 'reward';
prtpreds(5).r = 0;
prtpreds(5).g = 0;
prtpreds(5).b = 255;

% prtpreds(14).name = '14 abort';
prtpreds(6).name = 'abort';
prtpreds(6).r = 255;
prtpreds(6).g = 0;
prtpreds(6).b = 0;
...
prtpreds(2).onset = states_onset_fix_nostim(states_fix_nostim == 3) + stim_offset + 1; % from time corresponding to end of microstimulation
prtpreds(2).offset = states_onset_fix_nostim(states_fix_nostim == 20); % to success

% predictor: fixation with microstimulation 
prtpreds(3).onset = states_onset_fix_stim(states_fix_stim == 3) + stim_onset; % from beginning of microstimulation
prtpreds(3).offset = states_onset_fix_stim(states_fix_stim == 3) + stim_offset; % to end of microstimulation

% predictor: post-fixation with microstimulation
prtpreds(4).onset = states_onset_fix_stim(states_fix_stim == 3) + stim_offset + 1; % from end of microstimulation
prtpreds(4).offset = states_onset_fix_stim(states_fix_stim == 20); % to success

% predictor: reward
prtpreds(5).onset = states_onset_reward_fix(states_reward_fix == 20) + 1; % from beginning of reward period
prtpreds(5).offset = states_onset_reward_fix(states_reward_fix == 50) + reward_time; % to end of ITI

% predictor: abort
if ~isempty(states_onset_abort)
    prtpreds(6).onset = states_onset_abort(:,1);
    prtpreds(6).offset = states_onset_abort(:,2) + ITI_fail;
else
    prtpreds(6).onset = [];
    prtpreds(6).offset = [];
end
