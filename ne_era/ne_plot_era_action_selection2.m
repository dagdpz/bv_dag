function [era, anova, settings, compare] = ne_plot_era_action_selection2(voipath,avgpath,mdmpath,era_settings_id,save_figure,varargin)
...
if params.plot_summary
    ...
    species = 'Bacchus';
	%species = 'humans'; left_hemi_pattern = 'LH_';
	
	% remove irrelevant conditions
	if strcmp(species,'Bacchus'), data_anova = data_anova(:,[3 4 5 6]); left_hemi_pattern = '_LH'; end % for Bacchus
    
	for v = 1:size(data_anova,1)
		
		if ~isempty(strfind(era.voi(v).Name,left_hemi_pattern)),
			hemi = 0;
		else
			hemi = 1;
        end    
        ...
        area_n           = era.voi(v).Name;
        idx_under_lines  = strfind(area_n,'_');
        area_n           = area_n(idx_under_lines(1)+1:end);
		voi_data(v).name = 		area_n;
		
	end

	Area = {'a46','a45','F5','SMA','PMd','PMm','a24d','insula','M1','area5','S1','AIP','MIP','LIPd','LIPv','FST','TPO','MT','V4v','V1','PuM','MD'};
...
% SET THIS
dist = 0.06; % distance between significance markers
