function [era, anova, settings, compare] = ne_plot_era_fmri_microstim(voipath,avgpath,mdmpath,era_settings_id,save_figure,varargin)
...
if params.do_anova,
	...
	if ~isempty(settings.assignment)
		...
		if isfield(settings,'conditions2take') && ~isempty(settings.conditions2take)
...
for v = 1:size(data1,1)
	...
	if size(data_anova_2take,2) > 1 && ~isempty(anova) % only if there are more than two conditions
		...
		ve = version('-release');
		if str2num(ve(1:4)) < 2014, % for older MATLAB versions (< 2014), multcompare returns 5 columns, 6 column - p-value - is missing
			% replace by ttest2
			for i = 1:size(comparisons,1),
				[h,p] = ttest2(data_anova{v,settings.condition_numbers(comparisons(i,1))}, data_anova{v,settings.condition_numbers(comparisons(i,2))} );
				comparisons(i,6) = p;
			end
		end
    ...
    if ~isempty(params.plot_graphics)
        ...
        if ~isempty(settings.plot_diff), % plot difference between (some) curves, or sums of some curves
            for d=1:size(settings.plot_diff,1)
                D(d,:) = settings.plot_diff_fun(data1(v,[settings.plot_diff{d}{1}{:}],:), 2) - settings.plot_diff_fun(data1(v,[settings.plot_diff{d}{2}{:}],:), 2);
                c(1,d) = settings.plot_diff{d}{1}{1};
                
                if isfield(settings,'plot_diff_color') && ~isempty(settings.plot_diff_color) % replace avg colors with custom colors
        ...
        if strcmp(params.plot_graphics,'bars') || strcmp(params.plot_graphics,'both'),
            ...
            for e = 1:length(anova(v).p) % number of main/interaction effects
                text(0.5,ylim(2)-((ylim(2)-ylim(1))/15+offset_text),sprintf('F(%s)=%0.3f, p=%0.4f', anova(v).table{1+e,1},anova(v).table{1+e,6},anova(v).table{1+e,7}),'FontSize',8)
                offset_text = offset_text+(ylim(2)-ylim(1))/15;
            end
            text(0.5,ylim(2)-((ylim(2)-ylim(1))/15+offset_text),sprintf('alpha_t = %e',settings.alpha_post_hoc_t),'FontSize',8)
            
            if ~isempty(comparisons)
                offset_line = 0;
                for comp = 1:size(comparisons,1)
                    if comparisons(comp,6) <= settings.alpha_post_hoc_t
                        idx_max = comparisons(comp, means(comparisons(comp,1:2),1) == max(means(comparisons(comp,1:2),1)));
                        x_line = settings.condition_numbers(comparisons(comp,1:2)); % numbering of conditions is different in barplot and post-hoc comparisons
        ...
        if ~isempty(save_figure) && params.open_figure
            ...
            for f = 1:length(hf),
                ...
                if isfield(settings,'font_size_axes') && ~isempty(settings.font_size_axes)
...
if params.plot_summary
    ...
    if strcmp(species,'Bacchus'), data_anova = data_anova(:,[3 4 5 6]); end % for Bacchus
    
    for v = 1:size(data_anova,1)
        
        if ~isempty(strfind(era.voi(v).Name,'_LH')),
            hemi = 0;
        else
            hemi = 1;
        end
        ...
        voi_data(v).name = 		era.voi(v).Name;
        
    end
    
    if strcmp(species,'Bacchus'),
        % Bacchus
        Area = {'a46','a45','F5','SMA','PMd','PMm','a24d',...
            'insula','M1',...
            'area5','AIP','MIP','LIPd','LIPv',...
            'FST','TPO','MT','V4v','V1',...
            'PuM','MD',...
            };
    else
        ...
        Area = {'Middle_Frontal_Gyrus','Middle_Frontal_Gyrus_post',...
            'Inferior_Frontal_Gyrus','Claustrum','Cingulate_Gyrus','Medial_Frontal_Gyrus','Parahippocampal_Gyrus',...
            'Insula','M1',...
            'Precentral_Gyrus',...
            'Lingual_Gyrus',...
            'Thalamus','Culmen','Lentiform_Nucleus',...
            };
        
    end
    
    % 	col_pul		= [ 0.8706    0.4902         0];
    % 	col_frontal	= [ 1    0         0];
    % 	col_cd_ins_amy  = [1.0000    0.6000    0.6000];
    % 	col_sts		= [ 0.4000    0.8000    1.0000];
    % 	col_parietal	= [ 0.6000    0.6000    1.0000];
    % 	col_occipital	= [ 0.6000    1.0000         0];
    
    area_col =  [];
    
    for a = 1:length(Area),
        RH_a(a,:) = [NaN NaN NaN]; RH_p(a,:) = [1 1 1];
        LH_a(a,:) = [NaN NaN NaN]; LH_p(a,:) = [1 1 1];
        
        idx_cell = strfind(({voi_data.name}),([Area{a} '_RH']));
        idx = find(~cellfun(@isempty,idx_cell));
        if isempty(idx), % Human VOIs are named "hemisphere_area"
            idx_cell = strfind(({voi_data.name}),(['RH_' Area{a}]));
            idx = find(~cellfun(@isempty,idx_cell));
        end
        if ~isempty(idx),
            idx = idx(1); % take only first match (just in case there are mistankely more than one)
            RH_a(a,:) = voi_data(idx).a;
            RH_p(a,:) = voi_data(idx).p; % hand | space | interaction
            
        end
        idx_cell = strfind(({voi_data.name}),([Area{a} '_LH']));
        idx = find(~cellfun(@isempty,idx_cell));
        if isempty(idx),
            idx_cell = strfind(({voi_data.name}),(['LH_' Area{a}]));
            idx = find(~cellfun(@isempty,idx_cell));
        end
        
        if ~isempty(idx),
            idx = idx(1);
            LH_a(a,:) = voi_data(idx).a;
            LH_p(a,:) = voi_data(idx).p;
        end
        
    end
    
    ig_figure('Name',[params.load_mat],'Position',[200 200 600 850]);
    subplot(1,2,1)
    plot_barh(LH_a,Area,area_col,LH_p);
    xlabel('%BOLD change difference');
    title('LH');
    subplot(1,2,2)
    plot_barh(RH_a,Area,area_col,RH_p);
    set_axes_equal_lim;
    xlabel('%BOLD change difference');
    title('RH');
...
set(gca,'YTick',ytick,'YTickLabel',areas,'FontSize',10,'Ydir','reverse','Ylim',[0 n+1]);
