function [era, anova, settings, compare] = ne_plot_era_action_selection1(voipath,avgpath,mdmpath,era_settings_id,save_figure,varargin)
...
for v = 1:size(data1,1)
	...
	if size(data_anova_2take,2) > 1 && ~isempty(anova) % only if there are more than two conditions
		...
		v = version('-release');
		if str2num(v(1:4)) < 2014, % for older MATLAB versions (< 2014), multcompare returns 5 columns, 6 column - p-value - is missing
			% replace by ttest2
