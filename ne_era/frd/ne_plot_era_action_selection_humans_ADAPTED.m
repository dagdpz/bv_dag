function [era, anova, settings, compare] = ne_plot_era_action_selection_humans_ADAPTED(voipath,all_avgpaths,mdmpath,era_settings_id,save_figure,varargin)
...
for v = 1:size(data1,1)
    for a = 1:length(all_avgpaths)
        %% load AVG with plotting parameters
        clear avg
        avg = xff(all_avgpaths(a));
        settings.ResolutionOfDataPoints	= avg.ResolutionOfDataPoints;
        settings.NrOfTimePoints		    = avg.NrOfTimePoints;
        settings.PreInterval            = avg.PreInterval;
        settings.PostInterval           = avg.PostInterval;
        ...
        if ~isempty(params.plot_graphics)
            ...
            if (strcmp(params.plot_graphics,'tc') || strcmp(params.plot_graphics,'both')),
                ...
                N_subplots_h = length(all_avgpaths);
                N_subplots_v = 1;
                
                % get colors for errorbars from time courses
                ...
                for k = 1:N_subplots_h
                    ...
                    for i = c(k,:),
                        ...
                        plot(timeaxis,squeeze(data1(v,i,:)),...
                            'Color',[settings.TimeCourseColorR(i) settings.TimeCourseColorG(i) settings.TimeCourseColorB(i)]/255,...
                            'LineWidth',settings.TimeCourseThick(i)/1.5);
            ...
            if ~isempty(save_figure) && params.open_figure
                ...
                for f = 1:length(hf),
                    ...
                    for form = 1:length(save_figure)
                        ...
                        newdir = [fileparts(all_avgpaths) filesep 'figures_' save_figure{form}(2:end) '_NE'];
                        if ~exist(newdir,'dir')
                            [~,mess,~] = mkdir(newdir);
                            disp([newdir ' created']);
                        end
