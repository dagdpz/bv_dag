function [era, anova, timeaxis, params] = ne_era_new(voipath,avgpath,mdmpath,era_settings_id,varargin)
...
%
% Example:
%   voipath = 'D:\MRI\Curius\combined\microstim_20140122-20140226_5_3_250uA_nobaseline\11pred\microstim_20140122-20140226_5_3_250uA_nobaseline_TAL.voi';
...
defpar = { ...
    ...
    'roisize', 'double', 'nonempty', 0; ... 
        ...
        'assignment', 'double', [], [];... % assignment vector for ANOVA, if left empty, ANOVA will be skipped
        };
...
if ~isempty(era_settings_id), 
    ...
    params.assignment = settings.assignment; % assignment vector for ANOVA
end
...
for f = 1:avg.NrOfFiles
	...
	for c = 1:avg.NrOfCurves % for each curve
		...
		elseif ~isempty(find([curve(c).File.EventPointsInFile] == f-1)) % EventPointsInFile is 0-based
			...
			for r = 1:n_rois
                % disp(['ROI ' num2str(r)])
				perievents = extract_perievents(onsets,tc{f,1}(:,r),TR,avg,params.tc_interpolate);
				peri(r,c).perievents = [peri(r,c).perievents perievents]; % events from all files(runs) together
				[~, peri_run(r,f,c).mean, peri_run(r,f,c).se, peri_run(r,f,c).n_trials] = mean_psc(perievents,avg);
			end
...
timeaxis = [0:avg.NrOfTimePoints-1]*time_resolution - avg.PreInterval;
...
if ~isempty(params.assignment)
    ...
    for v = 1:n_rois
        anova(v) = nway_anova(data_anova(v,:), params.assignment, varnames, levnames);
    end
else anova = [];
end
...
if isempty(onsets),
	...
	if tc_interpolate, % e.g. to 5 ms, x200 times = TR/5
        ...
        % onsets_i = onsets_i(onsets_i-avg.PreInterval*TR/tc_interpolate>0 & onsets_i+avg.PostInterval*TR/tc_interpolate<=n_vol*TR/tc_interpolate);
        ...
        onsets_i = onsets_i(onsets_i-avg.PreInterval>0 & onsets_i+avg.PostInterval<=n_vol*TR/tc_interpolate);
        
        if numel(onsets_i) > 0            
            idx = repmat(onsets_i,avg.NrOfTimePoints,1) - repmat(fliplr([-(avg.PostInterval):avg.PreInterval]),length(onsets_i),1)';
            perievents = tci(idx);
        else perievents = NaN(avg.NrOfTimePoints,1);
        end
		...
		% find valid (complete trial) events
        onsets = onsets(onsets-avg.PreInterval>0 & onsets+avg.PostInterval<=n_vol);
